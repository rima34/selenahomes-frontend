...
import { Category, PropertyType } from "@/types/property-type";
...
interface PropertyFormData {
  ...
  // Used for Ready to Move and For Rent
  price: string;
  size: string;

  // Used only for Off Plan in the form.
  // They are submitted to the backend as "price" and "size".
  priceFrom: string;
  sizeFrom: string;
...
const CreatePropertyModal = ({
    ...
    const fetchData = async () => {
      ...
      try {
        const categoriesResponse =
          await propertyTypeService.getCategories({}, { limit: 100 });
      ...
      try {
        const response =
          await propertyTypeService.getPropertyTypes({}, { limit: 100 });
  ...
  useEffect(() => {
    ...
    const categoryPropertyTypeIds = Array.isArray(
      selectedCategory.propertyTypes
    )
      ? selectedCategory.propertyTypes.map((propertyType) =>
    ...
    const filtered = propertyTypesList.filter((propertyType) =>
      categoryPropertyTypeIds.includes(propertyType.id)
    );
  ...
  useEffect(() => {
    if (property && mode === "edit") {
      const isOffPlan = property.status === PropertyStatus.OFF_PLAN;
      ...
      setFormData({
        ...
        category: "",
        propertyTypes:
          property.propertyTypes?.map((propertyType) => propertyType.id) || [],
        status: property.status || "",

        // Regular properties
        price: !isOffPlan ? property.price?.toString() || "" : "",
        size:
          !isOffPlan && typeof property.size === "string"
            ? property.size
            : "",

        // Off-plan form labels
        priceFrom: isOffPlan ? property.price?.toString() || "" : "",
        sizeFrom:
          isOffPlan && typeof property.size === "string"
            ? property.size
            : "",
        ...
        sizeArea:
          typeof property.sizeArea === "string" ? property.sizeArea : "",
        beds: property.beds?.toString() || "",
        baths: property.baths?.toString() || "",
        locationIframe:
          typeof property.locationIframe === "string"
            ? property.locationIframe
            : "",
        handoverBy:
          typeof property.handoverBy === "string"
            ? property.handoverBy
            : "",
        paymentPlan: property.paymentPlan || "",
        completionDate: property.completionDate
          ? new Date(property.completionDate)
          : undefined,
        images: null,
        replaceImages: false,
      });
  ...
  const handleSubmit = async (event: React.FormEvent) => {
    ...
    if (formData.status === PropertyStatus.OFF_PLAN) {
      if (!formData.priceFrom || !formData.sizeFrom) {
        alert("Please fill in Price From and Size From");
        return;
      }
    ...
    try {
      ...
      /*
       * The backend now has only:
       * price
       * size
       *
       * For Off Plan, the UI says "Price From" and "Size From",
       * but we send those values as price and size.
       */
      if (formData.status === PropertyStatus.OFF_PLAN) {
        submitData.append("price", formData.priceFrom);
        submitData.append("size", formData.sizeFrom.trim());
      } else {
      ...
      if (formData.locationIframe.trim()) {
        submitData.append(
          "locationIframe",
          formData.locationIframe.trim()
        );
      }
      ...
      if (formData.completionDate) {
        submitData.append(
          "completionDate",
          formData.completionDate.toISOString()
        );
      }
  ...
  const handleInputChange = (
    ...
    value: string | Date | FileList | boolean | null | undefined
  ) => {
  ...
  const togglePropertyType = (propertyTypeId: string) => {
    setFormData((previousData) => {
      const isSelected =
        previousData.propertyTypes.includes(propertyTypeId);

      return {
        ...previousData,
        propertyTypes: isSelected
          ? previousData.propertyTypes.filter(
              (id) => id !== propertyTypeId
            )
          : [...previousData.propertyTypes, propertyTypeId],
      };
    });
  };
  ...
  const removePropertyType = (propertyTypeId: string) => {
    setFormData((previousData) => ({
      ...previousData,
      propertyTypes: previousData.propertyTypes.filter(
        (id) => id !== propertyTypeId
      ),
    }));
  };
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  ...
                  {loadingCategories ? (
                    ...
                    categoriesList.map((category) => (
                      <SelectItem
                        key={category.id}
                        value={category.id}
                        className="text-slate-100 focus:bg-amber-200/10 focus:text-amber-100"
                      >
                        {category.name}
                      </SelectItem>
                    ))
            ...
            <div className="md:col-span-2">
              ...
              <Popover
                open={isPropertyTypesOpen}
                onOpenChange={setIsPropertyTypesOpen}
              >
                ...
                <PopoverContent
                  className="w-full p-0 bg-slate-900 border-amber-200/20"
                  align="start"
                >
                  <div className="max-h-64 overflow-y-auto p-2">
                    {loadingPropertyTypes ? (
                      ...
                      <div className="p-4 text-center text-sm text-slate-400">
                        {formData.category
                          ? "No property types in this category"
                          : "Select a category first"}
                      </div>
                    ) : (
                      <div className="space-y-1">
                        {filteredPropertyTypes.map((propertyType) => (
                          <div
                            ...
                            onClick={() =>
                              togglePropertyType(propertyType.id)
                            }
                          >
                            <Checkbox
              ...
              {formData.propertyTypes.length > 0 && (
                <div className="flex flex-wrap gap-2 mt-2">
                  {formData.propertyTypes.map((typeId) => {
                    const propertyType = filteredPropertyTypes.find(
                      (item) => item.id === typeId
                    );
                    ...
                    return propertyType ? (
                      <Badge
                        key={typeId}
                        variant="secondary"
                        className="gap-1 bg-amber-200/10 text-amber-100 border-amber-200/20 hover:bg-amber-200/20"
                      >
                        {propertyType.name}
            ...
            <div>
              ...
              >
                ...
                <SelectContent className="bg-slate-900 border-amber-200/20">
                  <SelectItem
                    value={PropertyStatus.READY_TO_MOVE}
                    className="text-slate-100 focus:bg-amber-200/10 focus:text-amber-100"
                  >
                    Ready to Move
                  </SelectItem>
            ...
            {formData.status === PropertyStatus.OFF_PLAN && (
              <div>
                <Label htmlFor="priceFrom" className="text-slate-300">
                  Price From (AED) *
                </Label>
            ...
            {formData.status === PropertyStatus.OFF_PLAN && (
              <div>
                <Label htmlFor="sizeFrom" className="text-slate-300">
                  Size From *
                </Label>
                ...
                <Input
                  ...
                  onChange={(event) =>
                    handleInputChange("sizeFrom", event.target.value)
                  }
            ...
            {selectedCategoryName.toLowerCase() === "commercial" ? (
              <div>
                <Label htmlFor="sizeArea" className="text-slate-300">
                  Area
                </Label>
                ...
                <Input
                  ...
                  onChange={(event) =>
                    handleInputChange("sizeArea", event.target.value)
                  }
              ...
              <>
                <div>
                  <Label htmlFor="beds" className="text-slate-300">
                    Number of Beds
                  </Label>
                  ...
                  <Input
                    ...
                    value={formData.beds}
                    onChange={(event) =>
                      handleInputChange("beds", event.target.value)
                    }
                ...
                <div>
                  <Label htmlFor="baths" className="text-slate-300">
                    Number of Baths
                  </Label>
                  ...
                  <Input
                    ...
                    onChange={(event) =>
                      handleInputChange("baths", event.target.value)
                    }
            ...
            {formData.status === PropertyStatus.OFF_PLAN && (
              <>
                <div>
                  <Label htmlFor="handoverBy" className="text-slate-300">
                    Handover By
                  </Label>
                  ...
                  <Input
                    ...
                    onChange={(event) =>
                      handleInputChange("handoverBy", event.target.value)
                    }
                ...
                <div className="md:col-span-2">
                  <Label htmlFor="paymentPlan" className="text-slate-300">
                    Payment Plan
                  </Label>
                  ...
                  <Textarea
                    ...
                    onChange={(event) =>
                      handleInputChange("paymentPlan", event.target.value)
                    }
            ...
            <div className="md:col-span-2">
              ...
              <Input
                ...
                accept="image/*"
                onChange={(event) =>
                  handleInputChange("images", event.target.files)
                }
